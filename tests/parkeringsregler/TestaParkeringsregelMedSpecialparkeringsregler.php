...
class TestaParkeringsregelMedSpecialparkeringsregler extends TestCase
{
    ...
    public function test_lang_parkering_31_dagar_12_timmar()
    {
        $parkering = $this->skapaParkering( 1455750000+3600*8.75 ); // ursprunglig timestamp:  2016-02-18 00:00:00

        $parkering->avslutaParkering(1455750000+3600*24*31+12*3600); // Lägg till antal timmar (3600*t+sek)

        $parkering = Parkering::findOrFail($parkering->id);

        $this->assertEquals(25*31 + 5*3, $parkering->kostnad);
    }
}
